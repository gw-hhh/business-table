...
<ConfiguredBusinessTable
  ...
  :save-preference="savePreference"
...
原 Persistence 继续收发 schema1。ConfiguredBusinessTable 可通过 `savePreference: (value: PreferenceV3) => Promise<void>` 等待 schema3 写入成功，再发布设置与条件标记；宿主先读取 `preference` 并传回组件。`preferenceChange` 仍发出最新差量，但仅使用事件的旧接法不能确认外部保存成功。宿主负责存储隔离与版本冲突处理；本批不包含远端 409 冲突工作流。存储读取或保存异常会发诊断，Core 数据仍可用。

四项数据工具的定义、权限和状态归属见[数据工具接入与行为](07-数据工具接入与行为.md)。条件标记的可选规则随同一次配置事务写入 Preference v3，也可保存在 View；分组、对比和区域选择保持临时只读状态。
...
加载策略为 eager / after-definition / on-visible / on-interaction。列设置、组合筛选与四项数据工具默认 on-interaction；View 默认 after-definition。第一次激活才读取 details、创建上下文和加载默认 UI，后续复用。条件标记启用时的行投影在打开 UI 前就可生效。加载失败提供重试，关闭或卸载会丢弃迟到结果。

ESM 库与浏览器 Demo 拆分 UI 模块，未触发列设置或数据工具时不请求对应界面模块；分组和对比的 XLSX 写入模块在点击导出时才加载。UMD 保留旧单文件分发方式，不能提供独立网络分块；需要网络按需加载的宿主应使用 ESM 入口。列模板已有按需加载的富文本编辑器；服务端大数据导出仍待业务适配。
