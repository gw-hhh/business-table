...
> 3. 已有 `docs/01-项目入门/01-项目背景.md`
> 4. `docs/07-参考与归档/01-旧版迁移历史.md`
...
> 统一整理而成。
>
> 本文说明平台目标与总体设计；实际能力以当前状态、代码和验收为准。用户最新明确指令优先。分支工作流已按 2026-09-23 要求更新为 development 优先，详细执行规则见同目录《开发规则》。
...
7. 不机械把旧 JS 翻译成 Vue
8. 第一阶段先做完整差异分析和迁移矩阵
9. 在同步后的 development 继续实施，保留 main 基线
10. 默认直接在 development 开发；验证后按授权推送，正式发布另行安排
11. 新功能和 Bug 按 TDD
12. 每批必须跑：
    - vue-tsc
    - Vitest
...
本轮保留的 main 基线（2026-09-23 核对；后续以实际 Git 状态为准）：

```text
d06979f428cf52bd401a1e41589c190272474a56
```
...
保留 main，用户明确安排正式发布时再将已验证的 development 合入 main。默认不创建功能分支；确需功能分支时先获得用户相应安排。删除功能分支前确认所有提交已被 development 保留。
...
2. 对照旧版 reference
3. 对照当前 Vue 1.0.0
4. 更新当前状态、后续路线图和本轮版本记录；旧版迁移历史保持历史用途
5. 明确当前源码距离本文架构的差异
6. 不停留在方案
7. 在 development 继续开发，不从旧 main 重做
8. 从 P0 开始按 TDD 实际修改代码
9. 每完成一批跑完整 Release Gate
10. 验证通过后按授权推送 development；main 的发布合并另行安排
