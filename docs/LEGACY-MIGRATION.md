...
本轮更新：2026-09-23。当前进展见 [交互与视觉验收](LEGACY-INTERACTION-ACCEPTANCE.md)：视图、导出、记录抽屉、列菜单、设置预览与值/标签契约已按本轮实现更新；以下原始逐项矩阵保留为迁移历史，不应把其中“未实现”当成当前结果。

原始核查日期：2026-09-21。依据完整源码、样式、截图和本次实际运行结果，不能用功能名称或旧测试通过数代替 Vue 实现状态。
...
## 11. 2026-09-23：第一方筛选系统迁移

在既有 `feature/visual-parity-refinement@6225ced` 上继续实现，不退回 `79110eb`，不从 main/development 重新开始。本次实际新增第一方列筛选、嵌套 AND/OR 组合、方案生命周期、类型化候选项、独立条件标签、View 条件层快照与恢复。Core、列筛选和报价适配器共用纯函数谓词；编辑器状态不进入 Demo。

本地完整 `verify:release` 退出 0：**276/276 Vitest，69/69 Playwright（开发/生产），type-check、声明、库/Demo 构建、独立包消费者审计通过**。相对执行起点增加 56 项单元/集成断言用例和 16 项浏览器执行；原有测试没有跳过。Reference **145/145** 字节和 SHA-256 校验一致。
