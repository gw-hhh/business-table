...
基础搜索的摘要使用 `SearchContext.summaryItems`（已应用值），或直接使用导出的 `SearchSummary` 组件；不要把 `SearchContext.values` 中尚未提交的草稿拼成正式摘要。单选查询预设与完整示例见 [配置入口](CONFIGURATION.md#搜索显示值与实际值)。
