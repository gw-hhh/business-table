...
本轮更新：2026-09-23。当前进展见 [交互与视觉验收](../06-验收记录/2026-09/2026-09-23-旧版交互与视觉对齐/README.md)：视图、导出、记录抽屉、列菜单、设置预览与值/标签契约已按本轮实现更新；以下原始逐项矩阵保留为迁移历史，不应把其中“未实现”当成当前结果。
...
本节记录视觉还原前的架构前置批次；其中“本批”和未完成项均指当时状态，**最新完成项、门禁和剩余缺口见第9节**。需求优先级仍为：用户最新指示 → `docs/02-架构与规范/01-平台总体设计.md`（100节）→ 本矩阵中的产品完整性基准 → 旧规范中不冲突的约束。第6节原计划保留追踪用途。
...
完整结果及本批边界记录在[本地验收记录](../06-验收记录/2026-09/2026-09-21-配置运行时/README.md)，接入和兼容变化见[配置入口](../02-架构与规范/03-配置与能力规范.md)。
...
完整文件清单、验证与验收入口见 [docs/06-验收记录/2026-09/2026-09-21-报价页面样式还原/README.md](../06-验收记录/2026-09/2026-09-21-报价页面样式还原/README.md)。旧包145/145文件SHA-256再次核对一致；依赖版本和锁文件保持不变。构建保留既有UMD混合导出和包体积提示，不能将这些提示与浏览器运行错误混为一谈。
...
自查补测并修复：批量复制样式后的旧颜色输入缓存；只有非法颜色的草稿关闭确认；字体搜索继承表单padding的偏差；色板被预览区截断。阶段缺口仍见第9节。完整文件清单、验证范围和注意事项见 `docs/06-验收记录/2026-09/2026-09-22-设置细节精修/README.md`。产品修改仅本地提交，尚未推送到远端，不代表GitHub该分支已包含本轮UI修正。
...
完整范围、架构调整、已发现问题及风险：[docs/06-验收记录/2026-09/2026-09-23-筛选系统迁移/README.md](../06-验收记录/2026-09/2026-09-23-筛选系统迁移/README.md)。后续远端提交和 CI 结果以该分支实际 commit / workflow 为准，不把本地测试日志描述为远端已执行。
