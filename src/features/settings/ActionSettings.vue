...
const reorder=useDragReorder({ids:()=>ordered().map(action=>action.id),disabled:()=>props.disabled??false,move:(id,target)=>{const items=ordered();move(id,items.findIndex(item=>item.id===target)-items.findIndex(item=>item.id===id))}})
</script>
<template>
  <div class="bt-settings-page">
    ...
    <SettingsSection title="按钮及顺序">
      ...
      <SettingsSection v-for="(action,index) in ordered()" :title="modelValue.items[action.id]?.label??action.label" :key="action.id" class="bt-action-setting" :data-action-setting="action.id" v-bind="reorder.row(action.id)">
        ...
        <label class="bt-settings-check bt-action-setting__separator"><input type="checkbox" :checked="modelValue.items[action.id]?.separator??action.separator??false" @change="change(action.id,{separator:($event.target as HTMLInputElement).checked})">此按钮前加分隔线</label></fieldset>
