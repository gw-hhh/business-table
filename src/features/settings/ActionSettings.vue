...
import type { ActionPreference, RowActionLayout } from '../presentation/model'
import SegmentedControl from '../../ui/SegmentedControl.vue'
import TableIcon from '../../components/TableIcon.vue'
const props = defineProps<{ modelValue: RowActionLayout; actions: readonly Action[] }>()
...
function patch(value:Partial<RowActionLayout>){emit('update:modelValue',{...props.modelValue,...value})}
function change(id:string,value:ActionPreference){patch({items:{...props.modelValue.items,[id]:{...props.modelValue.items[id],...value}}})}
function ordered(){return [...props.actions].sort((a,b)=>(props.modelValue.items[a.id]?.order??a.order??0)-(props.modelValue.items[b.id]?.order??b.order??0))}
...
<template>
  <div class="bt-settings-page">
    ...
    <section class="bt-settings-section"><div class="bt-settings-section__heading"><h4>按钮及顺序</h4></div>
      <p v-if="!actions.length" class="bt-settings-note">当前表格没有注册操作按钮。</p>
      <div v-for="(action,index) in ordered()" :key="action.id" class="bt-action-setting" :data-action-setting="action.id" @dragover.prevent @drop.prevent="drop(action.id)">
        <div class="bt-action-setting__row">
          <button type="button" draggable="true" class="bt-settings-icon" :aria-label="'拖动操作 '+action.label" @dragstart="dragged=action.id"><TableIcon name="grip" :size="12"/></button>
          ...
          <div class="bt-setting-moves"><button v-for="offset in [-1,1]" :key="offset" class="bt-settings-icon" :aria-label="(offset<0?'上移 ':'下移 ')+action.label" :disabled="index+offset<0||index+offset>=actions.length" @click="move(action.id,offset)"><TableIcon :name="offset<0?'chevron-up':'chevron-down'" :size="12"/></button></div>
        </div>
