...
const props = defineProps<{ modelValue: Appearance; pageSizes?: number[] }>()
const emit = defineEmits<{ 'update:modelValue':[value:Appearance]; backup:[]; restore:[] }>()
function patch(value: Partial<Appearance>) { emit('update:modelValue', { ...props.modelValue, ...value }) }
</script>
<template>
  <div class="bt-settings-page">
    <section class="bt-settings-section"><div class="bt-settings-section__heading"><h4>默认文字</h4></div>
      <div class="bt-settings-grid bt-settings-grid--three">
        ...
        <label class="bt-settings-field"><span>内容字号</span><select aria-label="内容字号" :value="modelValue.fontSize" @change="patch({fontSize:Number(($event.target as HTMLSelectElement).value)})"><option v-for="size in [12,13,14,15,16,18,20,22,24,28,32]" :key="size" :value="size">{{size}} px</option></select></label>
        <label class="bt-settings-field"><span>表头字号</span><select aria-label="表头字号" :value="modelValue.headerFontSize" @change="patch({headerFontSize:Number(($event.target as HTMLSelectElement).value)})"><option v-for="size in [12,13,14,15,16,18,20,22,24,28,32]" :key="size" :value="size">{{size}} px</option></select></label>
    ...
    <section class="bt-settings-section"><div class="bt-settings-section__heading"><h4>布局</h4></div>
      <div class="bt-settings-grid bt-settings-grid--three">
    ...
    <section class="bt-settings-section"><div class="bt-settings-section__heading"><h4>设置备份</h4></div><p class="bt-settings-note">只包含列、排序、按钮和外观设置，不包含业务数据。恢复后仍需点击应用。</p><div class="bt-settings-inline"><button class="bt-ui-button" type="button" @click="emit('backup')">导出设置</button><button class="bt-ui-button" type="button" @click="emit('restore')">恢复设置</button></div></section>
  </div>
</template>
