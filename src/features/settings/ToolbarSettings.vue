...
function groupReorder(group:'page'|'table'){return useDragReorder({ids:()=>ordered(group).map(tool=>tool.id),disabled:()=>props.disabled??false,move:(id,target)=>{const items=ordered(group);move(group,id,items.findIndex(item=>item.id===target)-items.findIndex(item=>item.id===id))}})}
...
<template><div class="bt-settings-page bt-toolbar-settings">
  ...
  <SettingsSection v-for="group in (['page','table'] as const).filter(group=>localTools[group].length)" :key="group" :title="group==='page'?'页面工具栏':'表格工具栏'" :disabled="disabled">
    <div class="bt-tool-config"><div class="bt-tool-config__heading"><span>顺序</span><span>名称</span><span>显示位置</span><span>展示形式</span><span>固定</span><span>分隔</span><span>移动</span></div>
      <div v-for="(tool,index) in ordered(group)" :key="tool.id" class="bt-tool-config__row" v-bind="reorder[group].row(tool.id)">
        <button class="bt-settings-icon" v-bind="reorder[group].handle(tool.id)" :disabled="disabled" :aria-label="'拖动工具 '+tool.label"><TableIcon name="grip" :size="12"/></button>
        ...
        <label class="bt-settings-check"><input type="checkbox" :aria-label="'固定工具 '+tool.label" :disabled="tool.immutable" :checked="tool.immutable||(modelValue[group][tool.id]?.fixed??tool.fixed)" @change="change(group,tool.id,{fixed:($event.target as HTMLInputElement).checked})"><span class="bt-tool-config__mobile-label" aria-hidden="true">固定</span></label>
        <label class="bt-settings-check"><input type="checkbox" :aria-label="'分隔工具 '+tool.label" :checked="modelValue[group][tool.id]?.separator??tool.separator" @change="change(group,tool.id,{separator:($event.target as HTMLInputElement).checked})"><span class="bt-tool-config__mobile-label" aria-hidden="true">分隔</span></label>
