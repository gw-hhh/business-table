...
<template><div class="bt-settings-page bt-toolbar-settings">
  ...
  <SettingsSection title="通用设置" :disabled="disabled"><div class="bt-toolbar-settings__preference"><label class="bt-settings-check"><input type="checkbox" :checked="modelValue.followView" @change="patch({followView:($event.target as HTMLInputElement).checked})">工具栏随命名视图保存和切换</label><label class="bt-settings-field"><span>工具间距</span><select aria-label="工具间距" :value="modelValue.gap" @change="patch({gap:Number(($event.target as HTMLSelectElement).value)})"><option v-for="gap in [0,2,4,6,8,12,16]" :key="gap" :value="gap">{{gap}} px</option></select></label></div></SettingsSection>
