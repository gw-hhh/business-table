...
import type { ToolDefinition, ToolPreference, ToolbarLayout } from '../presentation/model'
import TableIcon from '../../components/TableIcon.vue'
const props=defineProps<{modelValue:ToolbarLayout;tools:{page:readonly ToolDefinition[];table:readonly ToolDefinition[]}}>()
const emit=defineEmits<{'update:modelValue':[value:ToolbarLayout]}>()
function patch(value:Partial<ToolbarLayout>){emit('update:modelValue',{...props.modelValue,...value})}
...
function ordered(group:'page'|'table'){return [...props.tools[group]].sort((a,b)=>(props.modelValue[group][a.id]?.order??a.order??0)-(props.modelValue[group][b.id]?.order??b.order??0))}
...
<template><div class="bt-settings-page">
  ...
  <section v-for="group in (['page','table'] as const)" :key="group" class="bt-toolbar-settings__section"><h4>{{group==='page'?'页面工具栏':'表格工具栏'}}</h4>
    <div class="bt-tool-config"><div class="bt-tool-config__heading"><span>顺序</span><span>名称</span><span>显示位置</span><span>展示形式</span><span>固定</span><span>分隔</span><span>移动</span></div>
      <div v-for="(tool,index) in ordered(group)" :key="tool.id" class="bt-tool-config__row" @dragover.prevent @drop.prevent="drop(group,tool.id)">
        <button class="bt-settings-icon" draggable="true" :aria-label="'拖动工具 '+tool.label" @dragstart="dragged=tool.id"><TableIcon name="grip" :size="12"/></button>
        ...
        <label class="bt-settings-check"><input type="checkbox" :aria-label="'固定工具 '+tool.label" :disabled="tool.immutable" :checked="tool.immutable||(modelValue[group][tool.id]?.fixed??tool.fixed)" @change="change(group,tool.id,{fixed:($event.target as HTMLInputElement).checked})"><span>固定</span></label>
        <label class="bt-settings-check"><input type="checkbox" :aria-label="'分隔工具 '+tool.label" :checked="modelValue[group][tool.id]?.separator??tool.separator" @change="change(group,tool.id,{separator:($event.target as HTMLInputElement).checked})"><span>分隔</span></label>
        <div class="bt-setting-moves"><button v-for="offset in [-1,1]" :key="offset" class="bt-settings-icon" :aria-label="(offset<0?'上移工具 ':'下移工具 ')+tool.label" :disabled="index+offset<0||index+offset>=tools[group].length" @click="move(group,tool.id,offset)"><TableIcon :name="offset<0?'chevron-up':'chevron-down'" :size="12"/></button></div>
      </div><p v-if="!tools[group].length" class="bt-settings-note">当前区域没有注册工具。</p>
    </div>
  </section>
</div></template>
