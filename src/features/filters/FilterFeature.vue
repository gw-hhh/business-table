...
import { createFilterDraft, parseFilterDraft, createFilterGroupDraft, parseFilterGroupDraft, readFilterState, type FilterGroupDraft, type FilterRuleDraft } from './editor'
import { createFilterPlans } from './plans'
...
const busy = ref(false), error = ref(''), nameOpen = ref(false), planName = ref(''), planError = ref(''), selectedPlan = ref(''), renameMode = ref(false), deleteOpen = ref(false)
const plans = props.context.persistence ? createFilterPlans(props.context.tableKey, props.context.persistence) : undefined
...
function loadPlan(id: string) {
  const plan = planItems.value.find(plan => plan.id === id)
  if (!plan) return
  try {
    const snapshot = guardFilterState(readFilterState(plan), props.context.columns)
    const draft = createFilterGroupDraft(props.context.columns, snapshot.filterGroup)
    group.value = draft; draftColumns.value = snapshot.columnFilters; selectedPlan.value = id; unavailable.value = false; error.value = ''
  } catch (cause) { error.value = cause instanceof Error ? cause.message : String(cause) }
}
  ...
  <DialogFrame :title="single ? '筛选 · ' + (column?.title ?? '字段已不可用') : '组合筛选'" :subtitle="single ? '与当前查询及其他列条件同时生效。' : '基础查询、列筛选和这里的组合条件共同生效。'" :drawer="!single" :busy="busy" class="bt-filter-dialog" @close="context.close">
    ...
    <template v-else>
      <div v-if="plans" class="bt-filter-plans">
        <label class="bt-ui-field">筛选方案<select :value="selectedPlan" aria-label="筛选方案" :disabled="plans.loading.value || busy" @change="loadPlan(($event.target as HTMLSelectElement).value)"><option value="">选择方案</option><option v-for="plan in planItems" :key="plan.id" :value="plan.id">{{plan.name}}</option></select></label>
        <div class="bt-filter-plan-actions"><button type="button" class="bt-ui-button text" :disabled="busy || !plans.ready.value || unavailable" @click="nameDialog(false)">保存为方案</button><template v-if="selectedPlan"><button type="button" class="bt-ui-button text" :disabled="busy" @click="savePlan(true)">更新方案</button><button type="button" class="bt-ui-button text" :disabled="busy" @click="nameDialog(true)">重命名</button><button type="button" class="bt-ui-button text" :disabled="busy" @click="deleteOpen = true">删除方案</button></template></div>
        <p v-if="planLoadError" class="bt-ui-error" role="alert">{{planLoadError}} <button type="button" class="bt-ui-button text" :disabled="plans.loading.value" @click="reloadPlans">重新读取</button></p>
        <p v-if="draftColumns.length" class="bt-ui-note">包含 {{draftColumns.length}} 个列筛选，应用时一并生效。</p>
      </div>
