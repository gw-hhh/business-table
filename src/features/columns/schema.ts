...
export const mappingItemSchema=z.object({id:z.string().max(160).optional(),value:primitive,label:z.string().max(400),color:color.optional(),background:color.optional(),border:color.optional(),icon:z.string().max(40).optional()}).strict()
...
export const contentSchema=z.object({wrap:z.enum(['ellipsis','two','wrap']).optional(),emptyText:z.string().max(400).optional(),description:z.string().max(1000).optional(),copyable:z.boolean().optional(),link:z.boolean().optional(),secondaryField:z.string().max(160).optional(),showSecondary:z.boolean().optional(),dateFormat:z.enum(['iso','slash','cn']).optional()}).strict()
const attrs=z.object({bold:z.boolean().optional(),italic:z.boolean().optional(),underline:z.boolean().optional(),strike:z.boolean().optional(),color:color.optional(),background:color.optional(),font:z.enum(columnFontFamilies).optional(),size:z.string().regex(/^(10|11|12|13|14|15|16|18|20|22|24|28|32)px$/).optional(),align:z.enum(['left','center','right']).optional(),list:z.enum(['ordered','bullet']).optional(),link:z.string().max(1000).optional()}).strict()
