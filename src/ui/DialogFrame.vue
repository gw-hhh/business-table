  ...
  <Teleport to="body">
    <div class="bt-dialog-overlay" :class="{'bt-dialog-overlay--drawer': drawer}" @pointerdown.self="close">
      <section ref="dialog" v-bind="$attrs" class="bt-dialog" :class="{ 'bt-dialog--wide': wide, 'bt-dialog--drawer': drawer }" role="dialog" aria-modal="true" :aria-label="title" tabindex="-1" @keydown="keydown">
        <header class="bt-dialog-header"><div><h2>{{ title }}</h2><p v-if="subtitle">{{ subtitle }}</p></div><button type="button" class="bt-ui-icon" :aria-label="'关闭'+title" :disabled="busy" @click="close"><TableIcon name="close" /></button></header>
...
.bt-dialog-overlay--drawer{padding:0;justify-content:flex-end}.bt-dialog--drawer{width:min(580px,100vw);height:100dvh;max-height:100dvh;border-radius:0;animation:bt-drawer-enter .18s ease-out}.bt-dialog--drawer .bt-dialog-body{flex:1}@keyframes bt-drawer-enter{from{transform:translateX(24px);opacity:.5}to{transform:none;opacity:1}}@media(prefers-reduced-motion:reduce){.bt-dialog--drawer{animation:none}}@media(max-width:640px){.bt-dialog-overlay--drawer{padding:0}.bt-dialog--drawer{max-height:100dvh}}
</style>
