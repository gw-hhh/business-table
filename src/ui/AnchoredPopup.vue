...
const props = withDefaults(defineProps<{ anchor: HTMLElement | null; label: string; role?: 'menu' | 'dialog'; width?: number | 'content'; minWidth?: number; maxWidth?: number; popupClass?: string; initialFocus?: 'first'|'last' }>(), { role: 'menu', width: 230, initialFocus:'first' })
...
function scrolled(event: Event) { if (!popup.value?.contains(event.target as Node) && !childPopups.contains(event.target as Node)) close() }
...
<template><Teleport to="body"><div ref="popup" class="bt-anchored-popup" :class="popupClass" :role="role" :aria-label="label" tabindex="-1" :style="{...position,width:width==='content'?'max-content':width+'px',minWidth:minWidth===undefined?undefined:`min(${minWidth}px, calc(100vw - 16px))`,maxWidth:maxWidth===undefined?undefined:`min(${maxWidth}px, calc(100vw - 16px))`}" @keydown="keydown"><slot /></div></Teleport></template>
<style>
