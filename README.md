...
报价 Demo 已开放“数据工具 → 组合筛选／工具栏设置”。组合筛选支持 AND/OR、嵌套条件组、方案管理、统一查询摘要和 View 保存恢复。声明 `query-summary` 可将基础查询、列筛选和组合条件放在同一行；“清除条件”一次重置查询，基础项遵循 `resetBehavior`。本批范围及验证见 [组合筛选验收](docs/06-验收记录/2026-09/2026-09-23-组合筛选与查询摘要对齐/README.md)。条件标记、分组汇总、记录对比、区域选择和设置历史继续暂缓。
