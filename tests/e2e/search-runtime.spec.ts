...
test('quotation Search keeps drafts separate and saves ID-based values in a View', async ({ page }) => {
  ...
  await expect(page.locator('.bt-search-summary__chip')).toHaveText('客户：泽临管道')
...
test('preset Search labels survive View restore while storage retains the actual choice value', async ({ page }) => {
  await page.goto('/')
  await page.locator('#quotation-status').selectOption('open')
  await page.getByRole('button', { name: '查询', exact: true }).click()
  await expect(rows(page)).toHaveCount(4)
  await expect(page.locator('.bt-search-summary__chip')).toHaveText('状态：未结（草稿 / 评审中）')
  await expect(page.locator('.bt-search-summary')).toHaveCSS('min-height', '44px')
  await page.getByRole('button', { name: '保存与切换视图' }).click()
  await page.getByRole('button', { name: '另存为视图', exact: true }).click()
  const editor = page.getByRole('dialog', { name: '保存视图', exact: true })
  await editor.getByRole('textbox', { name: /视图名称/ }).fill('未结查询')
  await editor.getByRole('button', { name: '保存', exact: true }).click()
  const saved = await page.evaluate(() => JSON.parse(localStorage.getItem('business-table.quotation-demo.views.v1') ?? '[]') as {name:string;search?:{values:Record<string,unknown>};filters?:unknown[]}[])
  expect(saved.find(view => view.name === '未结查询')?.search?.values.status).toBe('open')
  expect(saved.find(view => view.name === '未结查询')?.filters).toEqual([])
  await page.reload()
  await page.getByRole('button', { name: '保存与切换视图' }).click()
  await page.getByRole('button', { name: '未结查询', exact: true }).click()
  await expect(rows(page)).toHaveCount(4)
  await expect(page.locator('.bt-search-summary__chip')).toHaveText('状态：未结（草稿 / 评审中）')
  await page.getByRole('button', { name: '移除状态：未结（草稿 / 评审中）', exact: true }).click()
  await expect(rows(page)).toHaveCount(6)
})
