...
test('the default example enables every available settings page and column section',async({page})=>{
  ...
  const pages=page.getByRole('tablist',{name:'表格设置分类'})
  await expect(pages.getByRole('tab')).toHaveText(['列设置','排序规则','操作按钮','表格外观','工具栏'])
  await page.getByRole('button',{name:'编辑列 金额',exact:true}).click()
  const sections=page.getByRole('navigation',{name:'列设置内容'})
  await expect(sections.getByRole('button')).toHaveText(['基本','内容','筛选','映射','数字','模板','试算'])
  ...
  await pages.getByRole('tab',{name:'操作按钮',exact:true}).click()
  await expect(page.getByRole('textbox',{name:'查看按钮名称',exact:true})).toBeEnabled()
  await pages.getByRole('tab',{name:'表格外观',exact:true}).click()
  await expect(page.getByRole('combobox',{name:'表格行高',exact:true})).toBeEnabled()
  await pages.getByRole('tab',{name:'工具栏',exact:true}).click()
...
test('settings are loaded on click and explicitly read-only controls remain visible',async({page})=>{
  ...
  await page.getByRole('button',{name:'更多设置',exact:true}).click()
  await page.getByRole('tablist',{name:'表格设置分类'}).getByRole('tab',{name:'表格外观',exact:true}).click()
  ...
  await page.getByRole('tablist',{name:'表格设置分类'}).getByRole('tab',{name:'列设置',exact:true}).click()
