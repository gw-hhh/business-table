...
for (const width of [320, 390]) {
  test(`explicit More preferences persist while wrapped tools stay visible at ${width}px`, async ({ page }) => {
    await page.goto('/')
    ...
    const sortDialog = page.getByRole('dialog', { name: '表格设置', exact: true })
    await expect(sortDialog.getByRole('tab', { name: '排序规则', exact: true })).toHaveAttribute('aria-selected', 'true')
    await sortDialog.getByRole('tab', { name: '排序规则', exact: true }).focus()
    await page.keyboard.press('Escape')
    ...
    await expect(page.locator('.bt-tool-item[data-tool-id="density"]')).toHaveCount(0)
    await page.getByRole('button', { name: '更多表格工具', exact: true }).click()
    await expect(page.getByRole('menuitem', { name: '行高密度', exact: true })).toBeVisible()
    await page.keyboard.press('Escape')
    await page.reload()
    await page.setViewportSize({ width, height: 700 })
    await expect(page.locator('.bt-tool-item[data-tool-id="reload"] button')).toBeVisible()
